...
class TopPiHoleSummaryClientsDataStatsViewModelTest {
    ...
    fun `WHEN viewmodel is initialized THEN uiResult should emit Loading state initially`() =
        runTest {
            // Given
            coEvery { fetchTopClientsUseCase() } returns flowOf(Result.success(mockk(relaxed = true)))
    ...
    fun `GIVEN usecase returns success WHEN viewmodel is initialized THEN uiResult should emit Loaded state`() =
        runTest {
            ...
            val expectedResultData = TopClientQueriesEntity(all = topClients, blocked = emptyList())
            coEvery { fetchTopClientsUseCase() } returns flowOf(Result.success(expectedResultData))
            ...
            viewModel.uiResult.test {
                ...
                assertThat(uiState.data).isInstanceOf(TopClientStatsContract.UiState::class.java)
                assertThat(uiState.data.topQueries.all).isEqualTo(topClients)
                assertThat(uiState.data.topQueries.blocked).hasSize(0)
                cancelAndConsumeRemainingEvents()
            }
        }
