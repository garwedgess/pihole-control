...
class FetchTopPiHoleSummaryClientsDataUseCaseTest {
    ...
    fun `invoke - emits success when repository returns data`() = runTest {
        val topClients = TopClientQueriesEntity(
            all = listOf(
                TopClientEntity("192.168.1.2", 120, 50f),
                TopClientEntity("192.168.1.3", 95, 35f),
        ...
        coEvery {
            ...
            flow {
                val fetchData = arg<suspend (ConnectionEntity) -> Result<TopClientQueriesEntity>>(0)
