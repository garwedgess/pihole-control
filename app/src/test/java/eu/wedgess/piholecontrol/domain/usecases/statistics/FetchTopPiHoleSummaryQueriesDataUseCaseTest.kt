...
class FetchTopPiHoleSummaryQueriesDataUseCaseTest {
    ...
    fun `invoke - emits success when repository returns data`() = runTest {
        val topQueries = TopQueriesEntity(
            allowed = listOf(
                TopDomainEntity(domain = "example.com", hits = 120, 60f),
                TopDomainEntity(domain = "test.com", hits = 80, 40f)
            ),
            blocked = listOf(
                TopDomainEntity(domain = "ads.com", hits = 95, 60f),
