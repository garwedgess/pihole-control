...
class PiHoleSummaryStatusV6DataMapperTest {
    ...
    fun `toStatusEntity - maps DISABLED correctly`() {
        val response = PiHoleStatusResponse(PiHoleStatus.DISABLED)
        val result = response.toStatusEntity()
    ...
    fun `toStatusEntity - maps UNKNOWN correctly`() {
        val response = PiHoleStatusResponse(PiHoleStatus.UNKNOWN)
        val result = response.toStatusEntity()
